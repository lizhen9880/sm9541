...
static float baro9541;             /* actual pressure */  
static float temp9541;          /* actual temperature */
...
*******************************************************************************/  
static rt_err_t sm95_read_adc(rt_sensor_t psensor, float* p9541, float* t9541)
{
	rt_uint8_t  buf95_read[4] = {0};
    rt_uint8_t  status;
    rt_err_t ans;
    float dat = 0.0f;
    rt_int32_t baro_adc95;       /* pressure adc */
    rt_int32_t temp_adc95;    /* temperature adc */
    ...
    if(ans == RT_EOK)
    {
        ...
        if(status == 0)
        {
            baro_adc95 = ((buf95_read[0]&0x3f)<<8) | buf95_read[1];
            temp_adc95 = (buf95_read[2]<<3) | (buf95_read[3]>>5);
              
            dat = (float)( psensor->info.range_max  - psensor->info.range_min ) /10;
            *p9541 = (baro_adc95 - SM9541_MINCOUNT)*dat / (SM9541_MAXCOUNT - SM9541_MINCOUNT) + psensor->info.range_min/10;
            *t9541 = (float)temp_adc95*200/2047 - 50;
            return RT_EOK;
        }           
    }
    return RT_ERROR;
...
static rt_size_t sm95_fetch_data(struct rt_sensor_device *psensor, void *buf, rt_size_t len )
{
    ...
    if(len > 0)
    {
        if(psensor->config.mode == RT_SENSOR_MODE_POLLING) 
        {
            ...
            {
                /* actual pressure */            
                if(sm95_read_adc(psensor, &baro9541, &temp9541) == RT_EOK)
                { 
                    sensor_data->type = RT_SENSOR_CLASS_BARO;
                    sensor_data->data.baro = baro9541*SM95_BARO_X_N;
                    sensor_data->timestamp = rt_sensor_get_ts();
                    if(len > 1)
                    {
                        sensor_data++;
                        sensor_data->type = RT_SENSOR_CLASS_TEMP;                
                        sensor_data->data.temp = temp9541*SM95_TEMP_X_N; 
                        sensor_data->timestamp = rt_sensor_get_ts();
                    }                         
                    else
                    {
...
static int rt_hw_sm9541_port(void)
{
    ...
    cfg.intf.dev_name = SM9541_I2C_BUS;            /* i2c bus */
    cfg.intf.user_data = (void *)SM9541_ADDR;      /* i2c slave addr */
